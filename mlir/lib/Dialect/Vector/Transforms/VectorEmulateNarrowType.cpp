...
struct ConvertVectorLoad final : OpConversionPattern<vector::LoadOp> {
  ...
  matchAndRewrite(vector::LoadOp op, OpAdaptor adaptor,
    ...
    // Here only the 1-D vector load is considered, and we also need to
    // linearize the memref types.
    // For example, to emulate i4 to i8, the following op:
    //
    ...
    auto newLoad = linearizeMemrefLoad(loc, sourceType, srcBits, dstBits,
    ...
    // To deal with the odd number of elements, extract the first original
    // number of elements after bit-casting.
    if (origElements % scale != 0) {
      auto extractOp = rewriter.create<vector::ExtractStridedSliceOp>(
          loc, bitCast, /*offsets=*/0, origElements, /*strides=*/1);
